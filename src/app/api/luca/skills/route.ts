...
const SKILLS = [
  ...
  },
  {
    ...
    notes: [
      ...
      "If LUCA_SURPLUS_BUYER_KEY is set, inference costs settle from Luca's own wallet on-chain",
    ],
  },
  {
    id: "surplus-challenge",
    name: "Surplus SIWE Challenge",
    description: "Step 1 of Luca's Surplus self-registration flow. Returns the SIWE challenge message Luca must sign with its private key to obtain a persistent inf_ buyer key. Sign the returned message on Hermes, then POST { message, signature } to surplus-buyer-key.",
    endpoint: "/api/v1/luca/surplus/challenge",
    method: "GET",
    input: {
      address: { type: "string", required: true, description: "Luca's 0x wallet address on Base" },
    },
  },
  {
    id: "surplus-buyer-key",
    name: "Surplus Buyer Key",
    description: "Step 2 of Luca's Surplus self-registration. POST the signed SIWE message here to obtain a persistent inf_ buyer key. Store the returned key as LUCA_SURPLUS_BUYER_KEY in Hermes environment so inference costs settle from Luca's wallet on-chain.",
    endpoint: "/api/v1/luca/surplus/buyer-key",
    method: "POST",
    input: {
      message: { type: "string", required: true, description: "The SIWE message returned by surplus-challenge" },
      signature: { type: "string", required: true, description: "Ethereum signature of the message from Luca's private key" },
    },
  },
  {
    id: "surplus-status",
    name: "Surplus Account Status",
    description: "Check Luca's Surplus buyer account: USDC balance, current allowance to the settlement contract, and active inf_ keys. Uses LUCA_SURPLUS_BUYER_KEY if set, falls back to SURPLUS_API_KEY.",
    endpoint: "/api/v1/luca/surplus/status",
    method: "GET",
    input: {
      key: { type: "string", required: false, description: "Optional: override which inf_ key to query" },
    },
  },
  {
    id: "surplus-prices",
    name: "Surplus Prices",
    description: "Current Surplus inference pricing: cost per token per model in USDC, settlement rate, and any active promotions.",
    endpoint: "/api/v1/luca/surplus/prices",
    method: "GET",
    input: {},
  },
  {
    id: "surplus-models",
    name: "Surplus Models",
    description: "List of models available through Surplus inference, including model IDs, context windows, and capability flags.",
    endpoint: "/api/v1/luca/surplus/models",
    method: "GET",
    input: {},
  },
  {
