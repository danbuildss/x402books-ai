...
const STATUS_META: Record<AgentResult["status"], { label: string; color: string }> = {
  upserted: { label: "Upserted", color: "#4AE8A0" },
  skipped:  { label: "Skipped",  color: "#F4B942" },
  error:    { label: "Error",    color: "#F46060" },
};
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--ink)" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: 24 }}>
        ...
        {errorDiagnostic && (
          <div style={{ background: "var(--surface)", border: "1px solid #F4606040", borderRadius: 8, padding: "14px 18px", marginBottom: 20 }}>
            ...
            {errorDiagnostic.rpc_errors.length > 0 && (
              <div>
                <div style={{ fontSize: 11, color: "#F46060", marginBottom: 4 }}>RPC errors:</div>
                {errorDiagnostic.rpc_errors.map((e, i) => (
                  <div key={i} style={{ fontSize: 11, color: "#F46060" }}>• Page {e.page}: {e.error}</div>
                ))}
              </div>
            )}
        ...
        {report && (
          <>
            ...
            {report.summary.total_discovered === 0 && (
              <div style={{ background: "#F4B94218", border: "1px solid #F4B942", borderRadius: 8, padding: "10px 14px", marginBottom: 16, color: "#F4B942", fontSize: 13 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 10, marginBottom: 24 }}>
              {([
                { label: "Metadata",    pct: report.summary.metadata_coverage_pct,    count: report.summary.metadata_fetched,     color: "#4AE8A0",  note: "metadata fetched" },
                { label: "DID",         pct: report.summary.did_coverage_pct,         count: report.summary.agents_with_did,      color: "#8B7CF6",  note: "with DID" },
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
              {[
                ...
                { label: "Token contracts", value: report.summary.token_contracts_found, color: "#F46060" },
              ].map((s) => (
                        ...
                        return (
                          <tr key={agent.agent_id} style={{ borderBottom: "1px solid var(--line)", background: i % 2 === 0 ? "transparent" : "var(--bg)" }}>
                            ...
                            <td style={{ padding: "8px 10px" }}>
                              ...
                              {agent.error && (
                                <div style={{ fontSize: 10, color: "#F46060", marginTop: 2 }}>{agent.error}</div>
                              )}
                            </td>
                            <td style={{ padding: "8px 10px", fontSize: 11, color: agent.has_did ? "#8B7CF6" : "var(--muted)" }}>
                              {agent.has_did ? "✓" : "—"}
                            </td>
                            ...
                            <td style={{ padding: "8px 10px", fontSize: 12, color: agent.books_eligible ? "#4AE8A0" : "var(--muted)" }}>
